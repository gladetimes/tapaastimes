...
from ...download_utils import download_if_modified, download
...
class Command(BaseCommand):
    ...
    def do_stops(self, feed: gtfs_kit.feed.Feed) -> dict[str, StopPoint]:
        ...
        for stop in stops_to_create:
            admin_area_code = stop.atco_code[:3]
            if admin_area_code not in admin_areas:
                try:
                    admin_area = AdminArea.objects.get(atco_code=admin_area_code)
                    admin_areas[admin_area_code] = admin_area.id
                except AdminArea.DoesNotExist:
                    admin_areas[admin_area_code] = None
            if admin_areas[admin_area_code]:
                stop.admin_area_id = admin_areas[admin_area_code]
    ...
    def handle_zipfile(self, path):
        ...
        with (
            ...
            for line in feed.stop_times.itertuples():
                ...
                pickup_type = getattr(line, 'pickup_type', 0)
                pick_up = pickup_type != 1 if pd.notna(pickup_type) else True

                drop_off_type = getattr(line, 'drop_off_type', 0)
                set_down = drop_off_type != 1 if pd.notna(drop_off_type) else True
    ...
    def handle(self, *args, **options):
        ...
        path: Path = settings.DATA_DIR / "titsa_gtfs.zip"
        ...
        try:
                ...
                try:
                    ...
                    with zipfile.ZipFile(path, 'r') as zf:
                        if len(zf.namelist()) > 0:
                            self.stdout.write("Using existing TITSA GTFS file")
                        else:
                            path.unlink()  # Remove invalid file
                            raise Exception("Invalid zip file")
                except:
                    if not options["force"]:
                        self.stdout.write("Existing file is invalid, use --force to re-download")
                        return
                    path.unlink()  # Remove invalid file

            if not path.exists() or options["force"]:
                self.stdout.write(f'Downloading TITSA GTFS feed from {source.url}')
                # Use curl for downloads, but be more tolerant of failures
                result = subprocess.run([
                    'curl', '-L', '-o', str(path), '--retry', '3', '--retry-delay', '5', source.url
                ], capture_output=True, text=True, timeout=600)

                if result.returncode != 0:
                    self.stdout.write(self.style.WARNING(f'curl failed: {result.stderr}'))
                    if path.exists():
                        self.stdout.write("Attempting to import partial download...")
                    else:
            ...
            try:
                ...
                raise Exception(f"Invalid GTFS file: {e}")

            self.stdout.write(f'Importing TITSA GTFS feed from {source.url}')

            try:
                with log_time_taken(logger):
                    self.handle_zipfile(path)
                self.stdout.write(self.style.SUCCESS('Successfully imported TITSA GTFS feed'))
            except (OSError, BadZipFile) as e:
                logger.exception(e)
                self.stdout.write(self.style.ERROR(f'Failed to import TITSA GTFS feed: {e}'))
        except Exception as e:
            logger.exception(e)
            self.stdout.write(self.style.ERROR(f'Failed to process TITSA GTFS feed: {e}'))
